package AEA4.ExceptionsExercicis.RegistreTemperatures;

import java.util.Scanner;

// Excepció Personalitzada
class NoTemperaturesException extends Exception {
    public NoTemperaturesException(String message) {
        super(message);
    }
}
...
public class RegistreTemperatures {
    ...
    public void mostrarMitjana() {
        ...
        } catch (NoTemperaturesException e) {
    ...
    public void calculaMitjana() throws NoTemperaturesException {
        try {
            if (numTemperatures == 0) {
                throw new NoTemperaturesException("No hi ha temperatures registrades per calcular la mitjana.");
            }
            float acumulador = 0;
            for (int i = 0; i < numTemperatures; i++) {
                acumulador += temperatures[i];
            }
            System.out.print((acumulador / numTemperatures));
        } catch (ArithmeticException e) {
            System.out.println("Error aritmètic en calcular la mitjana: " + e.getMessage());
        } catch (ArrayIndexOutOfBoundsException e) {
            System.out.println("Error d'accés a l'array: " + e.getMessage());
        }
    }

    public void calculaDiferencia() throws NoTemperaturesException {
        try {
            if (numTemperatures == 0) {
                throw new NoTemperaturesException("No hi ha temperatures registrades per calcular la diferència.");
            }
